...
## Tools NUEVAS (sin equivalente legacy — incorporadas en este backend)

Estas tools no existían en el backend anterior. Se crearon directamente en `conti-backend` como capacidades nuevas.

### Traducción async

| Tool | Descripción | Archivo | Estado |
| --- | --- | --- | --- |
| `start_markdown_translation` | Traduce un archivo Markdown chunk a chunk via `deep_translator` en background. Devuelve `job_id` inmediatamente. | `app/tools/translation_tools.py` | ✅ Activa |
| `get_translation_job` | Consulta estado y progreso de un job de traducción por `job_id`. Estados: `queued→running→completed/failed`. | `app/tools/translation_tools.py` | ✅ Activa |
| `list_translation_jobs` | Lista jobs recientes de traducción con estado y porcentaje de progreso. | `app/tools/translation_tools.py` | ✅ Activa |

### Documentos & PDF async

| Tool | Descripción | Archivo | Estado |
| --- | --- | --- | --- |
| `start_pdf_to_markdown` | Convierte PDF/DOCX/doc a Markdown con `markitdown` en background. Soporta `also_translate=true` para encadenar traducción automática. Devuelve `job_id`. | `app/tools/document_tools.py` | ✅ Activa |
| `get_md_conversion_job` | Consulta estado de conversión. Estados: `queued→running→completed_translating/completed/failed`. Incluye `md_path` y `translation_job_id` si aplica. | `app/tools/document_tools.py` | ✅ Activa |
| `list_md_conversion_jobs` | Lista jobs recientes de conversión PDF→MD. | `app/tools/document_tools.py` | ✅ Activa |

### RAG Flamehaven async

| Tool | Descripción | Archivo | Estado |
| --- | --- | --- | --- |
| `start_rag_ingest` | Ingesta un documento (URL o path) en Flamehaven RAG. Pipeline: descarga → markitdown → upload al store con `original_name` en metadata. Auth via `FLAMEHAVEN_API_KEY`. Devuelve `job_id`. | `app/tools/rag_tools.py` | ✅ Activa |
| `get_rag_ingest_job` | Consulta estado de ingestión. Estados: `queued→running→uploading→completed/failed`. Incluye `rag_response` con `filename`, `size_mb`, `request_id` de Flamehaven. | `app/tools/rag_tools.py` | ✅ Activa |
| `list_rag_ingest_jobs` | Lista jobs recientes de ingestión RAG. | `app/tools/rag_tools.py` | ✅ Activa |

**Configuración RAG** (`config/app_config.json` sección `rag`):
```json
{
...
**Stores creados en Flamehaven:**
- `default` — documentos generales
- `contamela` — documentos internos de Contamela (creado en primer ingest test)

**API key activa:** `key_c762e54f57fe` (permisos: `upload`, `search`, `stores`, `admin`; rate limit: 300 req/min)
